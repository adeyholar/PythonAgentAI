...
CHECK_INTERVAL = 5  # Check tasks every 5 seconds
OLLAMA_API_URL = "http://localhost:11434/api/generate"  # Matches your running Ollama
EMAIL_SERVER = "localhost"
EMAIL_PORT = 1025  # Local test server
...
class ChattyAgent:
    ...
    def _load_state(self):
        if os.path.exists(TASKS_FILE):
            ...
            print(f"No task file found at {TASKS_FILE}. Starting fresh.")
    ...
    def _generate_blog(self):
        """Generates a blog post using Ollama API with codellama:7b."""
        try:
            payload = {
                "model": "codellama:7b",  # Use available model
    ...
    def _send_email(self, subject, body):
        """Sends an email with the generated blog."""
        try:
            msg = MIMEText(body)
            msg["Subject"] = subject
            msg["From"] = EMAIL_FROM
    ...
    def _schedule_blog_generation(self):
        """Schedules blog generation and email every hour."""
        while True:
            now = datetime.now()
            next_hour = (now.hour + 1) % 24
            wait_time = (datetime(now.year, now.month, now.day, next_hour) - now).total_seconds()
            time.sleep(wait_time)
            blog_content = self._generate_blog()
            subject = f"Blog Post - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
            self._send_email(subject, blog_content)
            self._add_response_to_display(f"Blog generated and emailed at {datetime.now().strftime('%H:%M')}")
        ...
        if "hello" in command:
            ...
            if "(" in desc_with_priority and ")" in desc_with_priority:
                ...
                if "priority" in prio_part.lower() and ")" in prio_part:
                    ...
                    try:
                        priority = max(1, min(5, int(prio_value)))
                    except ValueError:
                        priority = 1
            else:
            ...
            if "schedule task" in action_part or "schedule recurring" in action_part:
                ...
                try:
                    new_priority = max(1, min(5, int(new_priority_str)))
                ...
                try:
                    ...
                    return {"action": "feedback", "suggestion": suggestion, "feedback": feedback, "response_text": response_text}
                except ValueError:
    ...
    def respond(self, command):
        ...
        print(f"Responding to: {nlu_result}")
        ...
        if nlu_result["action"] == "greet":
            ...
            self.task_history[nlu_result["desc"].lower()] += 1
